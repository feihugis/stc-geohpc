...
public class Utils {
    ...
    /**
     * @param inputBbox the input bbox, and this input should be in the format of (X11-X12,Y11-Y12,Z11-Z12,...),(X21-X22,Y21-Y22,Z21-Z22,...)
     * @return the bbox in the form of List
     * transfer the input bbox from the String format to Integer format
     */
    public static HashMap<String, List<Integer[]>> parseBbox(String inputBbox){
    	HashMap<String, List<Integer[]>> bboxMap = new HashMap<String, List<Integer[]>>();
    	List<Integer[]> startList = new ArrayList<Integer[]>();
    	List<Integer[]> endList = new ArrayList<Integer[]>();
    	List<String> bboxStringList = new ArrayList<String>();
    	
    	String template = "(\\d+-\\d+,){1,}\\d+-\\d+";
    	Pattern pattern = Pattern.compile(template);
    	Matcher matcher = pattern.matcher(inputBbox);
    	if(matcher.find()) {   				
    		int i = 1;
    		while(matcher.find(i)) {
    			String result = matcher.group(0);
    			bboxStringList.add(result);
    			i = i + result.length() + 3;
    			i = (i>inputBbox.length()? inputBbox.length():i) ;
    	...
    	for(String value : bboxStringList) {
    		int num = value.split(",").length;
    		Integer[] startCorner = new Integer[num];
    		Integer[] endCorner = new Integer[num];
    	...
    	bboxMap.put("start", startList);
    	bboxMap.put("end", endList);
    	return bboxMap;
    }
     ...
     * @param conf Configuration object for the current job
     * @return a MultiFileMode entry, specifying how to process multiple files
     */
    ...
    /**
     * Get a String indication whether the No Scan feature is enabled
     * @param conf Configuration object for the current job
    ...
    /**
     * Get a String indicating whether the Query Dependent partitioning
    ...
    /**
     * Parse a String indicating if Query Dependents partitioning 
     * should be used for this job
    ...
    /**
     * Determine if a set of coordinates are at, or beyond, the last valid 
     * data coordinate
    ...
    /**
     * Calculate the size of an n-dimensional array in bytes
     * @param array the n-dimensional array for which to calculate the total size
