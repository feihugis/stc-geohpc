...
import edu.gmu.stc.hadoop.index.io.input.reading.scheduler.MerraSplitScheduler;
...
public class MerraInputFormatByDBIndexWithScheduler extends FileInputFormat {
	private static final Log LOG = LogFactory.getLog(MerraInputFormatByDBIndexWithScheduler.class);
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException {
		...
		for(int i=0; i<startList.size(); i++) {
			   ...
			   //create varInfoList based on Index
			   if(numDims == 1) {
				   sTime = start[0];
				   eTime = end[0];
				 
				   //get variable records info from DB	  
				   for(String varName : varNamesList) {
					    	varInfoList.addAll(indexOptr.getVarInfoByTimeQuery(startDate, endDate, sTime, eTime, varName));
					  }	
					  
			    } else if(numDims ==2) {
				   for(int d1=start[0]; d1<=end[0]; d1++) {
					   DecimalFormat fmt = new DecimalFormat("00");
					   String s = fmt.format(d1) + fmt.format(start[1]); 
					   String e= fmt.format(d1) + fmt.format(end[1]);
					 
					   //get variable info from DB  
					   for(String varName : varNamesList) {
						    varInfoList.addAll(indexOptr.getVarInfoByTimeQuery(startDate, endDate, s, e, varName));
					   }	
				   }
			   }
		...
		try {
			...
			//Scheduler inputSplits
			int threadNum = Integer.parseInt(job.getConfiguration().get("threadNumPerNode"));
			if(threadNum < 1) {
				threadNum = 1;
				System.out.println("Input threadNumPerNode < 1");
			}
			MerraSplitScheduler merraSplitScheduler = new MerraSplitScheduler(inputSplits, threadNum);
			multiSplitList.addAll(merraSplitScheduler.reorganizeInputSplit());			
		...
		for(InputSplit multiSplit : multiSplitList) {
			resultList.add(multiSplit);
		}
		LOG.info("**************************  Create " + resultList.size() + " Splits spending " + (endTime - startTime)*1.0/1000.0 + " seconds");
		return resultList;
		//return inputSplits;
	...
	public static List<InputSplit> combineInputSplitByFilePath(List<InputSplit> inputSplitList) throws IOException, InterruptedException {
		List<ArrayBasedSplitByDBIndex> results = new ArrayList<ArrayBasedSplitByDBIndex>();
		int sum = inputSplitList.size();
		for(int i=0; i<sum; i++) {
			boolean isSame= false;
			ArrayBasedSplitByDBIndex inputSplit = (ArrayBasedSplitByDBIndex) inputSplitList.get(0);
			int j=0;
			for(j=0; j<results.size(); j++) {
				ArrayBasedSplitByDBIndex resultSplit = results.get(j);
				if( inputSplit.getFilePath().equals(resultSplit.getFilePath()) ) {
					isSame = true;
					break;
				}				
			}
		...
		List<InputSplit> resultList = new ArrayList<InputSplit>();
		for(ArrayBasedSplitByDBIndex split : results) {
	...
	public SpatioRange createSpatioRange(int[] startCorner, int[] endCorner) {
		int[] spatioStart = new int[2];
		int[] spatioEnd = new int[2];
		int[] spatioShape = new int[2];
		spatioStart[0] = startCorner[startCorner.length-2];
		spatioStart[1] = startCorner[startCorner.length-1];
		spatioEnd[0] = endCorner[endCorner.length-2];
		spatioEnd[1] = endCorner[endCorner.length-1];
		spatioShape[0] = spatioEnd[0] - spatioStart[0];
		spatioShape[1] = spatioEnd[1] - spatioStart[1];
		
		return new SpatioRange(spatioStart, spatioShape);
	}
	
	public SpatioRange createSpatioRangeBackup(int[] startCorner, int[] endCorner) {
	...
	public RecordReader createRecordReader(InputSplit inputSplit, TaskAttemptContext context) throws IOException, InterruptedException {
		...
		MultiMerraRecordReaderForDBindex recordReader = new MultiMerraRecordReaderForDBindex();
		return recordReader;
	}
