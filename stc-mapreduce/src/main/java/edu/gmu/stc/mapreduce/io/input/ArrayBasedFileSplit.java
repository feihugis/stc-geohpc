 ...
 * 
 * @author edu.ucsc.srl.damasc  adapted by Fei Hu
 */
public class ArrayBasedFileSplit extends InputSplit implements Writable {
 ...
 /* private Float _vmax = null;
  private Float _vmin = null;
  private Float _fillValue = null;*/
  
  private static final Log LOG = LogFactory.getLog(ArrayBasedFileSplit.class);
  
  public ArrayBasedFileSplit() {}
  
  public ArrayBasedFileSplit(Path path, String[] hosts) {
  ...
  */
  ...
  public void write(DataOutput out) throws IOException {
    ...
    //for hosts
    out.writeInt(this._hosts.length);
    for(int i=0; i< this._hosts.length; i++) {
    	Text.writeString(out, this._hosts[i]);
    }
  }
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    this._hosts = new String[length];
    for(int i=0; i< length; i++) {
    	this._hosts[i] = new String(Text.readString(in));
    }
