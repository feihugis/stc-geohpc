...
public class CombineMerraFileInputFormat extends FileInputFormat {
	private static final Log LOG = LogFactory.getLog(CombineMerraFileInputFormat.class);
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException {
		  ...
		  //MerraFileInputScheduler scheduler = new MerraFileInputScheduler(inputSplits, 4, 6);
		  
		  //List<InputSplit> combineSplits = new ArrayList<InputSplit>(scheduler.reOrganizeFileInput());
		  
		 /* int countt = 0;
		  for(int i=0; i<combineSplits.size(); i++) {
			  CombineArrayBasedFileSplit split = (CombineArrayBasedFileSplit) combineSplits.get(i);
			  int tt = split.getArrayBasedFileSplitList().size();
			  for(int j=0; j<split.getArrayBasedFileSplitList().size(); j++) {
				  countt++;
				  //LOG.info(countt + "_______" + split.getArrayBasedFileSplitList().get(j).toString() + "Type : " + split.getArrayBasedFileSplitList().get(j).getArraySpecList().get(0).getBboxType());			  
			  }
		  }*/
	  ...
	  public void buildIndexFiles(JobContext job, List<FileStatus> filesForIndex, String indexDir, String varDir) throws IOException {
		    ...
		    for(FileStatus file: files) {
		    	try { 
		  	      ...
		  	      if( numBlocks > 1 ) {
		  	    	  LOG.info("***********  File : " + path + " block size : " + numBlocks);
		  	      }
	...
	public void createVarInfoFile(String indexDir, List<Variable> varList, String fileName){
		  ...
		  for(Variable var : varList) {
			    ...
			    if(var.findAttribute("_FillValue") != null) {
			    	fillValue = (Float) var.findAttribute("_FillValue").getValue(0);
			    }
