...
public class STIndexUtils {
  public static List<String> getInputPaths(Configuration conf, String inputDir, String fileFormat) {
  ...
  public static List<String> getInputPaths(Configuration conf, String inputDir, String fileFormat, String filter) {
    List<String> files = new ArrayList<String>();
    Path path = new Path(inputDir);
    try {
      FileSystem fs = FileSystem.get(conf);
      ...
      while (ritr.hasNext()) {
        FileStatus file = ritr.next();
        String location = file.getPath().toString();
        if (location.endsWith(fileFormat) && location.contains(filter)) {
          fileStatusList.add(file);
          files.add(file.getPath().toString());
        } else {
          continue;
